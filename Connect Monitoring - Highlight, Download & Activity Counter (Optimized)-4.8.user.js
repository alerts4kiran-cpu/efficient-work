...
// @grant        GM_setValue
// @grant        GM_getValue
// @updateURL    https://raw.githubusercontent.com/alerts4kiran-cpu/efficient-work/main/Connect%20Monitoring%20-%20Highlight,%20Download%20&%20Activity%20Counter%20(Optimized)-4.8.user.js
// @downloadURL  https://raw.githubusercontent.com/alerts4kiran-cpu/efficient-work/main/Connect%20Monitoring%20-%20Highlight,%20Download%20&%20Activity%20Counter%20(Optimized)-4.8.user.js
...
(function() {
    ...
    const getActivitySummaryCSV = () => {
        const activityCounts = state.cachedActivityCounts;
        const sortedActivities = Object.keys(activityCounts).sort();
        let csv = '\r\r\r"=== ACTIVITY SUMMARY ==="\r"Activity","HC"\r';
        let totalCount = 0;
        for (let i = 0; i < sortedActivities.length; i++) {
            const activity = sortedActivities[i];
            const count = activityCounts[activity];
            totalCount += count;
            csv += `"${activity}",${count}\r`;
        }
    ...
    const getActivityCounts = () => {
        ...
        state.cachedActivityCounts = activityCounts;
    ...
    const updateActivitySummary = () => {
        ...
        for (let i = 0; i < sortedActivities.length; i++) {
            ...
            const count = activityCounts[activity];
            totalCount += count;
            rows.push(`<tr><td style="padding:8px;border-bottom:1px solid #ddd">${activity}</td><td style="padding:8px;border-bottom:1px solid #ddd;text-align:center;font-weight:bold">${count}</td></tr>`);
        }
        rows.push(`<tr><td style="padding:8px;font-weight:bold;background:#f0f0f0">Total</td><td style="padding:8px;font-weight:bold;background:#f0f0f0;text-align:center">${totalCount}</td></tr>`);
    ...
    const createActivitySummaryBox = () => {
        ...
        state.summaryBox.style.cssText = 'position:fixed;top:10px;left:10px;z-index:9999;background:#fff;padding:15px;border-radius:8px;box-shadow:0 4px 15px rgba(0,0,0,0.3);min-width:280px;max-width:400px;cursor:move;font-family:Arial,sans-serif';
        state.summaryBox.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;border-bottom:2px solid #FF9900;padding-bottom:8px"><h3 style="margin:0;color:#232F3E;font-size:16px">📊 Activity Summary</h3><div><button id="download-summary-btn" style="background:#FF9900;border:none;font-size:14px;cursor:pointer;color:#fff;padding:4px 8px;border-radius:4px;margin-right:8px;font-weight:bold" title="Download CSV">⬇</button><button id="close-summary-btn" style="background:none;border:none;font-size:20px;cursor:pointer;color:#666;padding:0;width:24px;height:24px">×</button></div></div><div style="max-height:400px;overflow-y:auto"><table style="width:100%;border-collapse:collapse;font-size:14px"><thead><tr style="background:#f8f8f8"><th style="padding:8px;border-bottom:2px solid #ddd;text-align:left;font-weight:bold">Activity</th><th style="padding:8px;border-bottom:2px solid #ddd;text-align:center;font-weight:bold">HC</th></tr></thead><tbody id="activity-summary-tbody"><tr><td colspan="2" style="padding:20px;text-align:center;color:#999">Loading...</td></tr></tbody></table></div><div style="margin-top:10px;padding-top:8px;border-top:1px solid #ddd;font-size:11px;color:#666;text-align:center">Updates every second • Drag to move</div>';
